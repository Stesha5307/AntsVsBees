...
D
        ...
        current = self.place
        while current:
            if current.bees and current.is_hive == False and distance >= self.lower_bound and distance <= self.upper_bound:
...
class FireAnt(Ant):
    ...
    def reduce_health(self, damage_taken: float):
        ...
        if current is not None:
            if self.health <= 0:
                damage_taken = damage_taken + self.damage
            bee_list = list(current.bees)
            for bee in bee_list:
    ...
    def action(self, gamestate: GameState):
        ...
        while current is not None:
            if current.ant is not None:
                if current.ant not in self.doubled_ants:
                    current.ant.double()
                    self.doubled_ants.append(current.ant)
                if current.ant.is_container and current.ant.ant_contained is not None:
                    if current.ant.ant_contained not in self.doubled_ants:
                        current.ant.ant_contained.double()
                        self.doubled_ants.append(current.ant.ant_contained)
